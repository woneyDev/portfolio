...
import { text as tx, sampleData as sd } from './wrapReportEmaOverseasData';
...
export default function WrapReportEmaOverseasScreen() {
    ...
    <>
      <DemoScreenShell screenRef={screenRef} scale={scale} naturalHeight={naturalHeight}>
        ...
        <section className={s.inner}>
          ...
          <h3 className={s.contTitSub} style={{ margin: '15px 0' }}>{L(tx.countryWeightTit)}</h3>
          <div className={s.chartFigure}>
            <AssetDonutChart data={sd.countryRows.map((r) => ({ name: L(r.name), value: r.weight }))} />
          </div>
